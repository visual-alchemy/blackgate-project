...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Enabling and starting Blackgate service..."
...
# ─── Disable this firstboot service so it won't run again ──────────────
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Disabling firstboot service..."
systemctl disable blackgate-firstboot.service
rm -f /etc/systemd/system/multi-user.target.wants/blackgate-firstboot.service

echo "[$(date '+%Y-%m-%d %H:%M:%S')] ✅ Blackgate first boot setup complete!"
