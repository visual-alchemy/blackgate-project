...
The Backup Download feature allows users to download a JSON backup of all routes and their destinations from the Blackgate system. This document explains how the feature works, its security considerations, and implementation details.
...
The downloaded backup file is a JSON file containing all routes with their destinations. The file is named `blackgate-routes-MM-DD-YY-HH:MM:SS.json` (with a timestamp) and has the following structure:
...
- **Selective Backup** — Allow users to select specific routes to back up
- **Different Formats** — Support additional formats like YAML or CSV
