...
fn parse_pat(data: &[u8], pmt_pid: &mut u32) -> bool {
    ...
    if offset + 4 > TS_PACKET_SIZE { return false; }
...
fn parse_pmt(data: &[u8], elementary_pid: &mut u32, stream_type: &mut u8) -> bool {
    ...
    if offset + 2 > TS_PACKET_SIZE { return false; }
...
fn parse_h264_sps(nal_data: &[u8]) -> Option<VideoInfo> {
    if nal_data.len() < 3 { return None; }

    let nal_unit_type = nal_data[0] & 0x1F;
    if nal_unit_type != 7 { return None; }
    ...
    let high_profile = matches!(profile_idc,
        100 | 110 | 122 | 244 | 44 | 83 | 86 | 118 | 128 | 138 | 139 | 134 | 135);
    if high_profile {
        chroma_format_idc = br.read_ue();
        if chroma_format_idc == 3 { br.read_bits(1); }
    ...
    let height = height.saturating_sub(
        (crop_top + crop_bottom) * (2 - frame_mbs_only_flag) * sub_h);
    ...
    if br.read_bits(1) != 0 {
        ...
        if ar_present != 0 {
            ...
            if ar_idc == 255 { br.read_bits(16); br.read_bits(16); }
        }
        // overscan
        if br.read_bits(1) != 0 { br.read_bits(1); }
        // video_signal_type
        if br.read_bits(1) != 0 {
            br.read_bits(4);
            if br.read_bits(1) != 0 { br.read_bits(8); br.read_bits(8); br.read_bits(8); }
        }
        // chroma_loc
        if br.read_bits(1) != 0 { br.read_ue(); br.read_ue(); }
        // timing
        if br.read_bits(1) != 0 {
...
fn parse_mpeg2_sequence(data: &[u8]) -> Option<VideoInfo> {
    ...
    let (fps_num, fps_den) = if frame_rate_code >= 1 && frame_rate_code <= 8 {
        (fps_num_table[frame_rate_code as usize], fps_den_table[frame_rate_code as usize])
    } else {
        (0, 1)
    };
...
fn parse_hevc_sps(nal_data: &[u8]) -> Option<VideoInfo> {
    if nal_data.len() < 4 { return None; }

    let nal_unit_type = (nal_data[0] >> 1) & 0x3F;
    if nal_unit_type != 33 && nal_unit_type != 34 { return None; }
    if nal_unit_type == 33 { return None; } // VPS — skip
    ...
    if max_sub_layers > 0 {
        for _ in max_sub_layers..8 { br.read_bits(2); }
    }
    for _ in 0..max_sub_layers {
        ...
        if sub_layer_level_present != 0 { br.read_bits(8); }
    }
    ...
    if br.read_bits(1) != 0 {
        ...
        if br.read_bits(1) != 0 { br.read_bits(4); }
    ...
    Some(VideoInfo {
        ...
        fps_num, fps_den, interlaced: false,
...
impl TsProbeState {
    ...
    pub fn probe(&mut self, buf: &gst::Buffer) -> Option<VideoInfo> {
        ...
        if map.len() < TS_PACKET_SIZE { return None; }
        if self.metadata_attempts > 500 { return None; }
        ...
        for p in 0..num_packets {
            ...
            if self.pmt_parsed && pid as u32 == self.elementary_pid {
                ...
                if self.pes_buffer.len() + payload_size <= MAX_PES_PAYLOAD {
                    self.pes_buffer.extend_from_slice(&ts[offset..offset + payload_size]);
                }
                ...
                let result = match self.video_stream_type {
                    STREAM_TYPE_H264 => {
                        self.scan_h264_start_codes()
                    }
                    STREAM_TYPE_HEVC => {
                        self.scan_hevc_start_codes()
                    }
                    STREAM_TYPE_MPEG2 => {
                        self.scan_mpeg2_start_codes()
                    }
                    _ => None,
                };
...
mod tests {
    ...
    fn test_parse_pat() {
        ...
        ts[8] = 0x00; ts[9] = 0x01; // transport_stream_id=1
        ts[10] = 0x00;      // version + current
        ts[11] = 0x00;      // section_number
        ts[12] = 0x00;      // last_section_number
        // Program 0x0001 → PMT PID 0x0064 (100)
        ts[13] = 0x00; ts[14] = 0x01; // program_number=1
        ts[15] = 0xE0; ts[16] = 0x64; // PMT_PID=100
        // CRC placeholder
        ts[17] = 0x00; ts[18] = 0x00; ts[19] = 0x00; ts[20] = 0x00;
